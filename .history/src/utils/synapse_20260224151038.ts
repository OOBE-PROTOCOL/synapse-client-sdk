...
/**
 * Internal helper to construct a `SynapseClientConfig` from a `SynapseEndpoint`
 * @type Param endpoint - The resolved SynapseEndpoint to use for RPC, WS, and gRPC URLs.
 * @type Param opts - Optional overrides for API key, timeout, debug, and headers.
 * @returns A `SynapseClientConfig` ready to be passed to `new SynapseClient(...)`.
 */
