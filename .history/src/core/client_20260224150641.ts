...
export class SynapseClient {
  ...
  static fromEndpoint(config: EndpointConnectConfig): SynapseClient {
    ...
    const { resolveEndpoint, toClientConfig } = require('../utils/synapse') as typeof import('@/utils/synapse');
    const endpoint = resolveEndpoint(config.network, config.region);
    const clientCfg = toClientConfig(endpoint, {
