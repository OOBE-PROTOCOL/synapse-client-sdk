/**
 * Synapse AI Tools — LangChain-compatible tools for every Solana RPC method.
 *
 * Usage:
 * ```ts
...
/** A single LangChain-compatible tool created from a Solana RPC method. */
export type SolanaTool = DynamicStructuredTool<any, any, any, any>;

/** A record keyed by camelCase method name → LangChain tool. */
  ...
  for (const method of agentRpcMethods) {
    ...
    const t = tool(
      async (input: Record<string, unknown>) => {
        try {
          const args = binding.argsFrom(input, transport);
          const result = await (binding.fn as Function)(...args);
    ...
    tools.push(t as SolanaTool);
    toolMap[method.name] = t as SolanaTool;
  }
