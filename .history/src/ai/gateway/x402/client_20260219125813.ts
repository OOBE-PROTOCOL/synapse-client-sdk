/**
 * x402 Client — Buyer-side x402 payment handler.
 *
...
/**
 * Signer function that takes a partially-built PaymentPayload
 * and returns a fully-signed one (with the signed transaction
 * in payload.transaction for Solana).
 *
...
/**
 * Selector function that picks the best payment requirements
...
/**
 * Budget tracker callback — called before paying to check
...
/** Configuration for the buyer-side x402 client. */
...
/** The outcome of a successful x402 payment flow. */
...
export class X402Client {
   ...
   * @returns Response + optional PaymentOutcome
   */
  async fetch(
    url: string,
    init?: RequestInit,
  ...
  /**
   * Process a raw 402 response (for integration with custom transports).
   *
   ...
   * @param body — response body (parsed JSON or raw string)
   * @returns { shouldRetry, paymentHeader } if payment was created
   */
  ...
  /**
   * Parse a PAYMENT-RESPONSE header from a successful response.
   */
  ...
  /** Total payments made in this client instance */
  get payments(): number {
    return this.paymentCount;
  }

  /** Total amount paid per asset (network:asset → bigint) */
  get totalAmountPaid(): ReadonlyMap<string, bigint> {
    return this.totalPaid;
  }

  /** Get total paid for a specific asset on a network */
...
/** Create an X402Client with the provided configuration. */
