 ...
 *
 * The developer provides the facilitator URL and optional auth headers.
 * Default facilitator: https://x402.org/facilitator (Coinbase)
 */
 ...
 *
 * 1. **Preset by name** — instant setup with a known Solana facilitator:
 *    ```ts
 *    const fac = createFacilitator('payai', { apiKey: 'pk_live_...' });
 *    const fac = createFacilitator(Facilitator.DEXTER, { apiKey: 'dex_...', devnet: true });
 *    ```
 *
 * 2. **Custom config** — full control:
 *    ```ts
 *    const fac = createFacilitator({ url: 'https://my-fac.com', ... });
 *    ```
 *
 * 3. **No args** — defaults to PayAI mainnet (no auth):
 *    ```ts
 *    const fac = createFacilitator();
 *    ```
 */
export function createFacilitator(
  configOrName?: Partial<X402FacilitatorConfig> | Facilitator | FacilitatorName,
  quickConfig?: FacilitatorQuickConfig,
): FacilitatorClient {
  // String → preset name
  if (typeof configOrName === 'string' && !('url' in (configOrName as object))) {
    // Check if it's a known facilitator name (not a URL)
    const resolved = resolveFacilitatorConfig(
      configOrName as Facilitator | FacilitatorName,
      quickConfig,
    );
    return new FacilitatorClient(resolved);
  }

  // Object → raw config
  return new FacilitatorClient(configOrName as Partial<X402FacilitatorConfig> | undefined);
}
