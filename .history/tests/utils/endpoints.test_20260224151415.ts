...
describe('resolveEndpoint', () => {
  ...
  it('resolves testnet:us', () => {
    ...
    expect(ep.rpc).toContain('testnet');
  });
...
describe('toClientConfig', () => {
  it('maps endpoint fields to client config', () => {
    ...
    expect(config.rpcUrl).toBe(ep.rpc);
    expect(config.wsUrl).toBe(ep.wss);
  });
...
describe('Legacy SYNAPSE_ENDPOINTS', () => {
  ...
  it('has mainnet and devnet entries', () => {
    expect(SYNAPSE_ENDPOINTS.mainnet).toBeDefined();
    expect(SYNAPSE_ENDPOINTS.devnet).toBeDefined();
  });

  it('mainnet has rpc, wss, and grpc fields', () => {
    const m = SYNAPSE_ENDPOINTS.mainnet;
