...
describe('ToolMarketplace', () => {
  ...
  describe('Reputation', () => {
    it('records attestation for reputation scoring', () => {
      ...
      marketplace.recordAttestation(seller1.id, true);
      marketplace.recordAttestation(seller1.id, true);
      marketplace.recordAttestation(seller1.id, false); // One failed

      const rep = marketplace.getReputation(seller1.id);
      expect(rep).toBeDefined();
      expect(rep!.totalAttestations).toBe(3);
      expect(rep!.verifiedAttestations).toBe(2);
    });
  });
