...
describe('AgentSession', () => {
  ...
  describe('Budget tracking', () => {
    ...
    it('throws BudgetExhaustedError when budget runs out', () => {
      // Make enough calls to exhaust the budget
      // Budget: 10_000, cost: 100 per call → 100 calls max
      // But maxCallsPerSession is 10, so we'll hit call limit first
      ...
      // Next call: budget is now 0, should throw
      expect(() => bigCostSession.preCall('getBalance')).toThrow(BudgetExhaustedError);
  ...
  describe('TTL expiry', () => {
    it('throws SessionExpiredError when TTL is exceeded', () => {
      ...
      const s = new AgentSession(intent, tier, sellerId, 1); // 1 second TTL
      s.activate();

      // Session was created 5 seconds ago with 1-second TTL → expired
