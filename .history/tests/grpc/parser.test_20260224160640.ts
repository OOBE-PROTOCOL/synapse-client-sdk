...
describe('Known programs registry', () => {
  ...
  it('resolves Token Program', () => {
    expect(resolveProgram('TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA')).toBe('Token Program');
  });
  ...
  it('isProgramInCategory identifies system programs', () => {
    expect(isProgramInCategory('11111111111111111111111111111111', SYSTEM_PROGRAMS)).toBe(true);
    expect(isProgramInCategory('11111111111111111111111111111111', JUPITER_PROGRAMS)).toBe(false);
  });
  ...
  it('getProgramCategory returns correct category', () => {
    ...
    expect(getProgramCategory('unknown_id')).toBeUndefined();
  });
...
describe('Transaction parsing', () => {
  it('decodes signature from raw buffer', () => {
    ...
    if (result.type === 'transaction') {
      ...
      expect(result.data.signature).toMatch(/^[1-9A-HJ-NP-Za-km-z]+$/);
      expect(result.data.signature.length).toBeGreaterThan(10);
    }
  });
